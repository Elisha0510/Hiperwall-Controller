	...
	class HitBoxPanel extends JPanel {
	   ...
	   class DraggablePanel extends JPanel {
	      ...
	      public DraggablePanel(String text) {
	         ...
	         Image scaledImage = originalImage.getScaledInstance(currentWidth, currentHeight, Image.SCALE_SMOOTH); //가져올 이미지를 월 한 칸 크기로 조절
	         ...
	         layeredPane.setPreferredSize(new Dimension(currentWidth, currentHeight)); //지정한 크기로 내부에 추가되는 컴포넌트들이 올바르게 표시되도록 설정
	         ...
	         addMouseListener(new MouseAdapter() {
	            ...
	            public void mouseReleased(MouseEvent e) { 
	               if (e.getComponent() instanceof DraggablePanel) { //이벤트 발생한 컴포넌트가 DraggablePanel인지 확인하기
	                  DraggablePanel releasedPanel = (DraggablePanel) e.getComponent(); //releasedPanel = 이벤트가 발생한 컴포넌트
	                  releasedPanel.setSize(releasedPanel.currentWidth, releasedPanel.currentHeight); //크기 조절을 끝낸 최종 컴포넌트 크기 갱신
	                  releasedPanel.label.setSize(releasedPanel.currentWidth, releasedPanel.currentHeight); //크기 조절을 끝낸 최종 컴포넌트의 이미지 크기 갱신
	                  revalidate();
	                  hitBoxIndex = -1;
	               }
	            }
	               ...
	               if (component instanceof DraggablePanel) { //이벤트 발생한 컴포넌트가 DraggablePanel인지 확인하기
	                  ...
	                  for (DraggablePanel panel : dragPanelOpenList) {
	                     if (panel.getName().equals(draggedPanel.getName())) { 
	                        panel.setSize(draggedPanel.getWidth(), draggedPanel.getHeight()); //dragPanelOpenList를 전부 순회해서 해당 패널을 찾아 속성 업데이트
	                        break;
	                     }
	                  }
	         ...
	         addKeyListener(new KeyListener() {
	             ...
	             @Override
	             public void keyPressed(KeyEvent e) { //빈주석
	                ...
	                // 좌우위아래 조절
	                if (e.getKeyCode() == KeyEvent.VK_DOWN && isSelected) { //빈주석
	                   int lastIndex = clickPanel.size() - 1;
	                   int beforePanel = 0;
	                   String panelID = "";
	         ...
	         addMouseWheelListener(new MouseWheelListener() { //스크롤이 움직이는 이벤트를 받으면 크기 변경이 발생
	            ...
	            public void mouseWheelMoved(MouseWheelEvent e) {
	               ...
	               //currentWidth와 currentHeight가 스크롤을 통해 변경될 새로운 너비와 높이(currentWidth * ratioW, currentHeight * ratioH)와 어떻게 다른지를 계산하는 변수
	               ...
	               dropSendSizeChangeXML(DraggablePanel.this.getName(), panelResult, ratioW, ratioH, subW, subH); // subW, subH: 크기 조절된 패널의 위치와 크기를 업데이트할 때 필요한 정보
	            }
	         });
	      }
	      ...
	      private void resizePanel(Point p) {
	         ...
	         dropSendSizeChangeXML(this.getName(), panelResult, ratioW, ratioH, 0, 0); //함수 호출해서 통합 컨트롤러에서 변경할 때마다 하이퍼월 컨트롤러도 변경
	         revalidate();
	      }
	      ...
	      private void dropSendSizeChangeXML(String id, int panelNumber, float currentWidthRatio,
	         ...
	         try {
	            ...
	            String xmlString = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\" ?>"; //xmlString 변수에다 변경값을 보낼 XML 명령 작성
	            xmlString += "<Commands>";
	            ...
	            xmlString += "<boundsh>" + (1080 * currentHeightRatio) + "</boundsh>"; //ratio값을 이용해 변경된 너비, 높이값을 하이퍼월 컨트롤러로 보내기
	            ...
	            if (requestConnection.getResponseCode() == 200) { // http 요청이 성공적으로 수행된 경우
	               for (openFileList item : openFileList) {
	                  if (item.id.equals(id)) {
	                     item.panelNumber = panelNumber; // 모든 openFileList를 순회했을 때 오픈된 파일의 id가 item의 id가 일치하는 컨텐츠가 새로운 패널로 옮겨짐
	                  }
	               }
	               ...
	               SwingUtilities.invokeLater(new Runnable() { //Swing 컴포넌트의 상태를 변경하는 경우 SwingUtilities.invokeLater 사용
	                  public void run() {
	                     ...
	                     for (DraggablePanel panel : dragPanelOpenList) { 
	                        if (panel.getName().equals(id)) { //모든 dragPanelOpenList를 순회했을 때 해당 리스트와 id가 일치하는 패널을 찾는 경우
	                     ...
	                     selectedPanel.setBounds(uiPointArray[0], uiPointArray[1],
	                           ...
	                           (int) (selectedPanel.getPreferredSize().height * currentHeightRatio)); //선택된 패널의 위치와 크기 설정하기
